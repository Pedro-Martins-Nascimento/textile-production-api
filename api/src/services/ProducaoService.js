...
const criarProducao = async (dados) => {
  ...
  if (qualidade<1||qualidade>3) throw new Error("A qualidade informada deve ser obrigatoriamente 1, 2 ou 3.");
...
const atualizarProducao = async (id, dados) => {
  const { data_producao, numero_tear, codigo_produto, quilos, pecas } = dados;
  let { qualidade, turno } = dados;
  ...
  const timestampSP = dataSP.replace(" ", "T");
  if (data_producao > timestampSP) {
    throw new Error(`A data de produção (${data_producao}) não pode ser uma data futura.`);
  }
  ...
  return await producaoRepository.atualizarProducao(id, {
    data_producao,
    numero_tear,
    codigo_produto,
    turno,
